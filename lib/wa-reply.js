...
async function handlePrescriptionRequest({ fromPhone, firstName, lang }) {
  ...
  try {
    ...
    return { reply: `Thanks${firstName ? ', ' + firstName : ''} — our team will share your prescription shortly. For anything urgent call 9963009090.`, action: 'handoff', source: 'rx_lookup_error' };
  }
  if (!lookup || lookup.patientCount === 0) {
    return { reply: `Hi${firstName ? ' ' + firstName : ''}, I couldn't find your records on this number. Please call the clinic at 9963009090 (Mon–Sat, 9 AM–8 PM) or book a visit at drsujeeth.com.`, action: 'handoff', source: 'rx_no_match' };
  }
  ...
  if (!lookup.publicLinkId) {
    return { reply: `Hi${firstName ? ' ' + firstName : ''}, I found your record but there's no shareable prescription on file yet. Please contact the clinic at 9963009090.`, action: 'handoff', source: 'rx_no_link' };
  }
  ...
  if (sent) {
    return { reply: `Here's your latest prescription${firstName ? ', ' + firstName : ''} — I've just sent the PDF to this number. Reply if you need anything else.`, action: 'sent_prescription', source: 'rx_resend' };
  }
  return { reply: `Sorry${firstName ? ', ' + firstName : ''}, I couldn't send your prescription just now — our team will follow up, or call 9963009090.`, action: 'handoff', source: 'rx_resend_failed' };
}
