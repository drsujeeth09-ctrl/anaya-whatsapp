...
import { prescriptionLookupByPhone, upcomingAppointmentsByPhone, formatTimeIST } from './db.js';
import { detectEscalation, sendEscalationAlert } from './escalation.js';
...
// Appended to the auto-reply when the message trips an escalation — honest
// because sendEscalationAlert really does fire right after (step 3b).
const CALLBACK_PROMISE = {
  en: `\n\nI've alerted our clinic team — you'll get a call back on this number soon.`,
  hi: `\n\nमैंने क्लिनिक टीम को सूचित कर दिया है — आपको जल्द ही इसी नंबर पर कॉल आएगा।`,
  te: `\n\nమా క్లినిక్ టీమ్‌కి తెలియజేశాను — త్వరలో ఈ నంబర్‌కే కాల్ వస్తుంది.`,
};
...
export async function handleInboundMessage(message, contacts) {
  ...
  if (messageType === 'button' || messageType === 'interactive') {
    ...
    inboundSummary = body;
    // "Confirm my appointment time" — answer from the EMR before the booking
    // patterns run, because BOOK_REGEX matches the word "appointment" and
    // would loop the patient with yet another booking link. STOP/emergency
    // keep priority.
    if (body && !STOP_REGEX.test(body) && !EMERGENCY_REGEX.test(body) && isAppointmentStatusQuery(body)) {
      decision = await handleAppointmentStatusRequest({ fromPhone, firstName });
    }
    if (!decision) decision = handleText(body, firstName);
  ...
  // -------------------------------------------------------------------------
  // 1b) Urgency check — decided BEFORE the reply goes out so we can append an
  //     honest "team has been alerted" promise (the alert really fires below).
  // -------------------------------------------------------------------------
  const escalation = detectEscalation({
    text: messageType === 'text' ? inboundSummary : '',
    action: decision?.action,
    source: decision?.source,
  });
  if (escalation.urgent && decision?.reply && decision.source !== 'appt_lookup_not_found') {
    // appt_lookup_not_found already promises the callback in its own copy.
    decision.reply += CALLBACK_PROMISE[detectLanguage(inboundSummary)] || CALLBACK_PROMISE.en;
  }
  ...
  // -------------------------------------------------------------------------
  // 3b) Fire the escalation alert to doctor + secretary (WhatsApp + email).
  //     Awaited (lambda-freeze rule) but never allowed to throw — the patient
  //     already got their reply. Throttled per patient inside sendEscalationAlert.
  // -------------------------------------------------------------------------
  let escalationResult = null;
  if (escalation.urgent) {
    escalationResult = await sendEscalationAlert({
      fromPhone,
      profileName,
      message: inboundSummary,
      reasons: escalation.reasons,
    }).catch(e => {
      console.error('[wa-reply] escalation alert failed:', e?.message);
  ...
  const summary = {
    ...
    escalated: escalation.urgent,
    escalationReasons: escalation.urgent ? escalation.reasons : undefined,
    escalationResult: escalationResult
      ? (escalationResult.skipped || escalationResult.error
          || `wa:${(escalationResult.whatsapp || []).filter(x => x.success).length}/${(escalationResult.whatsapp || []).length} mail:${(escalationResult.email || []).filter(x => x.success).length}/${(escalationResult.email || []).length}`)
      : undefined,
  };
...
// ---------------------------------------------------------------------------
// Appointment self-service: "confirm my appointment time" / "what time is my
// slot" → read the EMR and STATE what's booked (read-only, so listing every
// family member on a shared number is safe). Not-found is an escalation case:
// the patient believes they have a booking we can't see (missed Zoho webhook,
// paid-but-unsynced, or plain confusion) — a human must call them.
// ---------------------------------------------------------------------------

function isAppointmentStatusQuery(body) {
  const asksStatus = /\b(confirm|confirmed|what\s*time|when|timing|check)\b|कब|समय|कन्फर्म|ఎప్పుడు|సమయం|కన్ఫర్మ్/i.test(body);
  const aboutBooking = /\b(appointment|apointment|appt|booking|slot|time|visit|consult(ation)?)\b|अपॉइंटमेंट|बुकिंग|అపాయింట్|బుకింగ్/i.test(body);
  return asksStatus && aboutBooking;
}

async function handleAppointmentStatusRequest({ fromPhone, firstName }) {
  const tenDigit = String(fromPhone || '').replace(/[^0-9]/g, '').slice(-10);
  let rows;
  try {
    rows = await upcomingAppointmentsByPhone(tenDigit);
  } catch (e) {
    // Lookup infrastructure failure → fall through to the normal flow rather
    // than answering wrong.
    console.error('[wa-reply] appt lookup failed:', e?.message);
    return null;
  }
  if (!rows.length) {
    return {
      reply: `Namaste${firstName ? `, ${firstName} garu` : ''} 🙏 — I don't see an upcoming appointment on this number in our records, so I've alerted our clinic team to call you back and sort it out. If you'd like to book meanwhile, I'm sending the booking link too.`,
      action: 'send_booking_link',
      source: 'appt_lookup_not_found',
    };
  }
  const lines = rows.map(r => {
    const day = new Date(r.appt_date).toLocaleDateString('en-IN', {
      timeZone: 'Asia/Kolkata', weekday: 'short', day: 'numeric', month: 'short',
    });
    const time = formatTimeIST(r.start_time);
    const who = [r.first_name, r.last_name].filter(Boolean).join(' ') || 'Patient';
    const where = r.meet_link ? 'online video consult' : 'Apollo Clinic, Manikonda';
    return `• ${who}: ${day}${time ? ` at ${time}` : ''} — ${where}`;
  });
  return {
    reply: `Namaste${firstName ? `, ${firstName} garu` : ''} 🙏 Here's what I have booked:\n${lines.join('\n')}\n\nSee you then! Reply RESCHEDULE if you need a different time.`,
    action: null,
    source: 'appt_lookup_found',
  };
}
