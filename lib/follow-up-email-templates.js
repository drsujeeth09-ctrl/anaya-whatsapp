...
// panel with date / time / location rows, each prefixed by a tiny coloured
// label bar).  Used by the 24-hour appointment reminder.
function detailsPanelHtml({ dateLong, time, location }) {
 ...
 * appointment.
 */
export function renderAppointmentReminderEmail({ patient, appointmentDate, appointmentTime, bookingUrl }) {
  const firstName = patient?.firstName || 'there';
  ...
  const bodyHtml = `
    ...
    ${detailsPanelHtml({ dateLong, time, location: CLINIC.address })}
